...
const BookAppointment = () => {
  ...
  const ComingData = {
    name: "Dr Talal Zafar",
    speciality: "Cradiologist",
    phone: "0334779229",
    workingDetails: [
      { name: "Dow Medical Center", location: "ColumbiaDoctors - 1244 Amsterdam Avenue", fees: "2000" },
      { name: "Mount Sinai Hospital", location: "One Gustave L. Levy Place", fees: "2500" },
      { name: "Mayo Clinic", location: "200 First Street SW", fees: "3000" },
      { name: "Video Consultaion", location: "Video Consultaion", fees: "6000" },
    ],
    timingDetails: [
  ...
  const [selectedLocation, setSelectedLocation] = useState(ComingData.workingDetails[0].name);
  ...
  const address = ComingData.workingDetails.filter((e) => e.name === selectedLocation);
  ...
  const [modal,setModal]=useState({
    ...
    BookingTime:new Date().getHours()+":"+new Date().getMinutes()+":"+new Date().getSeconds(),

  })


  useEffect(() => {
    const selectedDay = dates.find((date) => date.date === selectedDate)?.day === "Today"
    ...
    if (selectedTiming) {
      ...
      const openingMinutes = Number(selectedTiming.openingTime.split(':')[1].split(' ')[0]);
  
      const closingTime = Number(selectedTiming.closingTime.split(':')[0]);
      const closingMinutes = Number(selectedTiming.closingTime.split(':')[1].split(' ')[0]);
      ...
      while (currentTime.getHours() < closingTime || (currentTime.getHours() === closingTime && currentTime.getMinutes() <= closingMinutes)) {
    ...
    <div className={classes.bg}>
      <Wrapper>
        <div className={classes.container}>
          <InfoBox name={ComingData.name} spec={ComingData.speciality} fees={address[0]?.fees} loc={address[0]?.location} phone={ComingData.phone} />
          <div className={classes.bookingBox}>
            ...
            <div className={classes.selectLoc}>
              ...
              <div className={classes.ScrollDateBox}>
                {dates.map((e, i) => (
                  <div key={i} onClick={() => { setSelectedDate(e.date);setSelectedMonth(e.month) }} className={selectedDate === e.date ? classes.SelectedDateBox : classes.DateBox}>
                    <p>{e.day}</p>
            ...
            <div className={classes.selectLoc}>
              ...
              <div className={classes.TimeBoxContainer}>
                {times.map((e, i) => (
                  <div key={i} onClick={() => { setSelectedTime(e) }} className={selectedTime === e ? classes.SelectedTimeBox : classes.TimeBox}><p>{e}</p></div>
                ))}
              </div>
            </div>
